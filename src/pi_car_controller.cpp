...
rclcpp::Publisher<std_msgs::msg::String>::SharedPtr msg_pub;
 ...
 void motor_control(float speed, float turn){
         ...
         if(left_velocity!=0||right_velocity!=0) {
                 RCLCPP_INFO(rclcpp::get_logger("pi_car_controller"), "left_in=%f, left_map=%f, right_in=%f, right_map=%f", left_velocity, linear_speed(left_velocity), right_velocity, linear_speed(right_velocity));
         }
...
void LED_timer_callback (){
 ...
 void velocity_callback(const geometry_msgs::msg::Twist msg)
 {
         ...
         if ((msg.linear.x>0)&& front_block) return;
         if ((msg.linear.x<0)&& back_block) return;
 ...
 int main (int argc, char **argv)
 {
         ...
         RCLCPP_INFO(rclcpp::get_logger("pi_car_controller"), "Started Pi Car Controller");
         nh = std::make_shared<rclcpp::Node>("pi_car_controller");
         signal(SIGINT, sigintHandler);
         signal(SIGTERM, sigintHandler);
         signal(SIGKILL, sigintHandler);
         if ((pi=pigpio_start(NULL,NULL))<0) {
                 RCLCPP_INFO(rclcpp::get_logger("pi_car_controller"), "gpio init failed");
                 return 1;
         }
